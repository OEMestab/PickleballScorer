package com.example.pickleballscorer

import android.content.Intent
import android.graphics.Color
import android.graphics.Typeface
import android.os.Bundle
import android.view.View
import android.widget.Button
import android.widget.Switch
import android.widget.TextView
import androidx.activity.enableEdgeToEdge
import androidx.appcompat.app.AppCompatActivity
import androidx.constraintlayout.widget.ConstraintLayout

data class GameState(
    val servingTeam: Int,
    val serverNumber: Int,
    val team1Score: Int,
    val team2Score: Int,
    val t1Right: String,
    val t1Left: String,
    val t2Right: String,
    val t2Left: String,
    val isGameOver: Boolean
)

class MainActivity : AppCompatActivity() {

    var servingTeam = 1
    var serverNumber = 2
    var team1Score = 0
    var team2Score = 0
    var isGameOver = false

    private var t1RightPlayer = ""
    private var t1LeftPlayer = ""
    private var t2RightPlayer = ""
    private var t2LeftPlayer = ""

    private val history = mutableListOf<GameState>()

    private var gameMode = ""
    private var t1p1 = ""
    private var t1p2 = ""
    private var t2p1 = ""
    private var t2p2 = ""

    private lateinit var tvTeam1Score: TextView
    private lateinit var tvTeam2Score: TextView
    private lateinit var tvPlayerTop: TextView
    private lateinit var tvPlayerBottom: TextView
    private lateinit var btnTeam1: Button
    private lateinit var btnTeam2: Button
    private lateinit var btnFault: Button
    private lateinit var btnRecall: Button

    private var defaultTextColor: Int = 0

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        setContentView(R.layout.activity_main)

        gameMode = intent.getStringExtra("GAME_MODE") ?: "SINGLES"
        servingTeam = intent.getIntExtra("STARTING_TEAM", 1)

        t1p1 = intent.getStringExtra("T1P1") ?: "P1"
        t1p2 = intent.getStringExtra("T1P2") ?: "P2"
        t2p1 = intent.getStringExtra("T2P1") ?: "P1"
        t2p2 = intent.getStringExtra("T2P2") ?: "P2"

        t1RightPlayer = t1p1
        t1LeftPlayer = t1p2
        t2RightPlayer = t2p1
        t2LeftPlayer = t2p2

        tvTeam1Score = findViewById(R.id.tvTeam1Score)
        tvTeam2Score = findViewById(R.id.tvTeam2Score)
        defaultTextColor = tvTeam1Score.currentTextColor

        tvPlayerTop = findViewById(R.id.tvPlayerTop)
        tvPlayerBottom = findViewById(R.id.tvPlayerBottom)

        btnTeam1 = findViewById(R.id.btnTeam1)
        btnTeam2 = findViewById(R.id.btnTeam2)
        btnFault = findViewById(R.id.btnFault)
        btnRecall = findViewById(R.id.btnRecall)

        findViewById<Switch>(R.id.switchGameMode).visibility = View.GONE

        if (gameMode == "SINGLES") {
            serverNumber = 1
        }

        btnTeam1.setOnClickListener {
            if (servingTeam == 1 && !isGameOver) {
                saveState()
                team1Score++
                if (gameMode == "DOUBLES") {
                    val temp = t1RightPlayer
                    t1RightPlayer = t1LeftPlayer
                    t1LeftPlayer = temp
                }
                checkWinCondition()
                updateScoreboardUI()
            }
        }

        btnTeam2.setOnClickListener {
            if (servingTeam == 2 && !isGameOver) {
                saveState()
                team2Score++
                if (gameMode == "DOUBLES") {
                    val temp = t2RightPlayer
                    t2RightPlayer = t2LeftPlayer
                    t2LeftPlayer = temp
                }
                checkWinCondition()
                updateScoreboardUI()
            }
        }

        btnFault.setOnClickListener {
            if (isGameOver) {
                launchWinnerScreen()
            } else {
                saveState()
                handleFault()
            }
        }

        btnRecall.setOnClickListener {
            if (history.isNotEmpty()) {
                val prevState = history.removeAt(history.size - 1)
                servingTeam = prevState.servingTeam
                serverNumber = prevState.serverNumber
                team1Score = prevState.team1Score
                team2Score = prevState.team2Score
                t1RightPlayer = prevState.t1Right
                t1LeftPlayer = prevState.t1Left
                t2RightPlayer = prevState.t2Right
                t2LeftPlayer = prevState.t2Left
                isGameOver = prevState.isGameOver

                updateScoreboardUI()
            }
        }

        updateScoreboardUI()
    }

    fun saveState() {
        history.add(GameState(servingTeam, serverNumber, team1Score, team2Score, t1RightPlayer, t1LeftPlayer, t2RightPlayer, t2LeftPlayer, isGameOver))
    }

    fun handleFault() {
        if (gameMode == "SINGLES") {
            servingTeam = if (servingTeam == 1) 2 else 1
        } else {
            if (serverNumber == 1) {
                serverNumber = 2
            } else {
                servingTeam = if (servingTeam == 1) 2 else 1
                serverNumber = 1
            }
        }
        updateScoreboardUI()
    }

    fun checkWinCondition() {
        if (team1Score >= 11 && (team1Score - team2Score) >= 2) isGameOver = true
        else if (team2Score >= 11 && (team2Score - team1Score) >= 2) isGameOver = true
    }

    fun launchWinnerScreen() {
        val winnerName = if (team1Score > team2Score) {
            if (gameMode == "SINGLES") t1p1 else "$t1p1 & $t1p2"
        } else {
            if (gameMode == "SINGLES") t2p1 else "$t2p1 & $t2p2"
        }

        val intent = Intent(this, WinnerActivity::class.java)
        intent.putExtra("WINNER_NAME", winnerName)
        startActivity(intent)
    }

    fun updateScoreboardUI() {
        tvTeam1Score.text = team1Score.toString()
        tvTeam2Score.text = team2Score.toString()

        if (isGameOver) {
            if (team1Score > team2Score) {
                tvTeam1Score.setTextColor(Color.GREEN)
                btnTeam1.text = "WINNER"
            } else {
                tvTeam2Score.setTextColor(Color.GREEN)
                btnTeam2.text = "WINNER"
            }
            btnFault.text = "See Results"
            return
        } else {
            tvTeam1Score.setTextColor(defaultTextColor)
            tvTeam2Score.setTextColor(defaultTextColor)
            btnTeam1.text = "+ T1"
            btnTeam2.text = "+ T2"
            btnFault.text = "Fault (Missed Rally)"
        }

        val topParams = tvPlayerTop.layoutParams as ConstraintLayout.LayoutParams
        val bottomParams = tvPlayerBottom.layoutParams as ConstraintLayout.LayoutParams

        if (gameMode == "SINGLES") {
            tvPlayerTop.text = "👤 $t1p1"
            tvPlayerBottom.text = "👤 $t2p1"

            if (servingTeam == 1) {
                // Corrected Top Player (Team 1) alignment: Even score = Left (0.25), Odd score = Right (0.75)
                // Receiver (Team 2) stands diagonally opposite
                if (team1Score % 2 == 0) {
                    topParams.horizontalBias = 0.25f  // Left side for Even
                    bottomParams.horizontalBias = 0.75f // Right side for Receiver
                } else {
                    topParams.horizontalBias = 0.75f  // Right side for Odd
                    bottomParams.horizontalBias = 0.25f // Left side for Receiver
                }
            } else {
                // Team 2 is serving (Bottom): Even score = Right (0.75), Odd score = Left (0.25)
                // Receiver (Team 1) stands diagonally opposite
                if (team2Score % 2 == 0) {
                    bottomParams.horizontalBias = 0.75f // Right side for Even
                    topParams.horizontalBias = 0.25f    // Left side for Receiver
                } else {
                    bottomParams.horizontalBias = 0.25f // Left side for Odd
                    topParams.horizontalBias = 0.75f    // Right side for Receiver
                }
            }

            tvPlayerTop.layoutParams = topParams
            tvPlayerBottom.layoutParams = bottomParams

            resetPlayerStyle(tvPlayerTop, t1p1)
            resetPlayerStyle(tvPlayerBottom, t2p1)

            if (servingTeam == 1) highlightServer(tvPlayerTop, t1p1)
            else highlightServer(tvPlayerBottom, t2p1)

        } else {
            // Doubles Mode Layout & Rotation Representation
            topParams.horizontalBias = 0.5f
            bottomParams.horizontalBias = 0.5f
            tvPlayerTop.layoutParams = topParams
            tvPlayerBottom.layoutParams = bottomParams

            val t1Display = "$t1RightPlayer & $t1LeftPlayer"
            val t2Display = "$t2RightPlayer & $t2LeftPlayer"

            resetPlayerStyle(tvPlayerTop, t1Display)
            resetPlayerStyle(tvPlayerBottom, t2Display)

            if (servingTeam == 1) {
                highlightServer(tvPlayerTop, t1Display)
            } else {
                highlightServer(tvPlayerBottom, t2Display)
            }
        }
    }

    private fun resetPlayerStyle(tv: TextView, name: String) {
        tv.text = "👤 $name"
        tv.setTextColor(Color.WHITE)
        tv.setTypeface(null, Typeface.NORMAL)
    }

    private fun highlightServer(tv: TextView, name: String) {
        tv.text = "🏓 $name"
        tv.setTextColor(Color.YELLOW)
        tv.setTypeface(null, Typeface.BOLD)
    }
}